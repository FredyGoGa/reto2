...
def ordenar_alfabeticamente(listaFinalProductos):
    listaFinalProductosOr = sorted(listaFinalProductos)
    print("lista ordenada" + listaFinalProductosOr)
    return listaFinalProductosOr
...
if __name__ == '__main__':
    ...
    #print(f'Se procesarán {cantidad} productos')
   
    #Recibe Informacion de cada producto
    rango_productos = range(cantidad)
    for n in rango_productos:
        info_producto = recibir_info_producto(n)
        listaProductos.append(info_producto)
        listaCodigo.append(info_producto.get("codigo"))
        listaNombre.append(info_producto.get("nombre"))
        listaCantidad.append(info_producto.get("cantidad"))
        listaValorUnitario.append(info_producto.get("valor_sin_iva"))
        listaTipoDeIva.append(info_producto.get("tipo_iva"))

    #print(f"Estos son los productos ingresados: {listaProductos}")
    print(len(listaCodigo))
    print(len(listaNombre))
    print(len(listaCantidad))
    print(len(listaValorUnitario))
    print(len(listaTipoDeIva))

    # Calcular valores asociados a cada producto
    #listaFinalProductos = list(map(calcular_valores_asociados, listaProductos))
    for producto in listaProductos:
        listaFinalProductos.append(calcular_valores_asociados(producto))
    #print(f"Estos son los productos ingresados con sus valores asociados: {listaFinalProductos}")

    # Presenta los datos de acuerdo a los requerimientos
    # del reto

    for producto in listaFinalProductos:
        ...
        iva_acumulado = iva_acumulado + producto.get('valor_iva_producto')
        valor_total_acumulado = valor_total_acumulado + producto.get('valor_total_producto')


    print(valor_total_acumulado)
    print(iva_acumulado)
